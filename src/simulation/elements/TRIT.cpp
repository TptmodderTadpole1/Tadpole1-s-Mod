...
void Element::Element_TRIT()
{
	...
	HeatConduct = 251;
	Description = "Tritium oxide. Acts like normal deut and reacts violently with deut.";
...
static int update(UPDATE_FUNC_ARGS)
		...
		for (auto rx = -1; rx <= 1; rx++)
		{
			for (auto ry = -1; ry <= 1; ry++)
			{
				if (rx || ry)
				{
					...
					if (TYP(r)==PT_DEUT && sim->rng.chance(1, 20))
					{
						sim->part_change_type(ID(i), x, y, PT_NEUT);
						sim->part_change_type(ID(r),x+rx,y+ry,PT_NBLE);
						parts[ID(i)].life = 680;
						parts[ID(i)].temp = MAX_TEMP;
						parts[ID(i)].temp = MAX_TEMP;
						sim->pv[y/CELL][x/CELL]+=800.0f;
						return 1;
					}
					if (!r || (parts[i].life >=maxlife))
						continue;
					if (TYP(r)==PT_TRIT&& sim->rng.chance(1, 3))
					{
			...
			for (auto rx = -1; rx <= 1; rx++)
			{
				for (auto ry = -1; ry <= 1; ry++)
				{
					if (rx || ry)
					{
						...
						auto r = pmap[y+ry][x+rx];
						if ((!r)&&parts[i].life>=1)//if nothing then create trit
						{
							auto np = sim->create_part(-1,x+rx,y+ry,PT_TRIT);
							if (np<0) continue;
							parts[i].life--;
	...
	for (auto trade = 0; trade<4; trade ++)
	{
		...
		if (rx || ry)
		{
			...
			if (!r)
				continue;
			if (TYP(r)==PT_TRIT&&(parts[i].life>parts[ID(r)].life)&&parts[i].life>0)//diffusion
			{
